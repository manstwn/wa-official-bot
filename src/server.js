...
import { saveMessage, readMessages, clearMessages } from './db.js';
    ...
    if (payload.tempMediaUrl && (payload.type === 'image' || payload.type === 'audio' || payload.type === 'video')) {
      try {
        ...
        if (localPath) {
          ...
          const recordIndex = messages.findIndex(m => m.id === payload.id);
          if (recordIndex !== -1) {
            messages[recordIndex].tempMediaUrl = localPath;
            const DB_FILE = path.join(path.resolve('data'), 'db.json');
            await fs.promises.writeFile(DB_FILE, JSON.stringify(messages, null, 2), 'utf-8');
            broadcast(messages[recordIndex]);
          }
        }
      } catch (dlErr) {
...
fs.promises.mkdir(DATA_DIR, { recursive: true }).catch(() => {});
...
async function readAIConfig() {
  try {
    const data = await fs.promises.readFile(AI_CONFIG_FILE, 'utf-8');
...
async function writeAIConfig(cfg) {
  const merged = { ...DEFAULT_AI_CONFIG, ...cfg };
  await fs.promises.writeFile(AI_CONFIG_FILE, JSON.stringify(merged, null, 2), 'utf-8');
  return merged;
}
...
// ============================================================
const AI_HISTORY_FILE = path.join(DATA_DIR, 'ai-history.json');

async function readAIHistory() {
  try {
    const data = await fs.promises.readFile(AI_HISTORY_FILE, 'utf-8');
    return JSON.parse(data);
  } catch { return []; }
}

async function writeAIHistory(history) {
  await fs.promises.writeFile(AI_HISTORY_FILE, JSON.stringify(history, null, 2), 'utf-8');
}
...
app.delete('/api/ai-history/:time', async (req, res) => {
  try {
    ...
    let history = await readAIHistory();
